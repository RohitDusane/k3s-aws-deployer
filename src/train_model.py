...
from src.config import METRICS_DIR, MODELS_DIR, PROCESSED_DATA_DIR, TARGET_COL
...
def train_and_evaluate() -> dict:
    ...
    if 1 not in classes:
        raise RuntimeError("Fraud class '1' not found")

    fraud_index = classes.index(1)
    y_proba = pipeline.predict_proba(X_test)[:, fraud_index]
